...
这里只关心 `descriptor` 的 `get` 和 `set`

#### initState

在 Vue 的初始化阶段，`_init` 方法执行的时候，会执行 `initState(vm)` 方法，它的定义在 `src/core/instance/state.js` 中。`initState` 方法主要是对 `props`、`methods`、`data`、`computed` 和 `wathcer` 等属性做了初始化操作。
...
- 通过 defineReactive 将 props 上的属性变成响应式
- 通过 proxy 将 vm._props 的属性代理到 vm 上
...
- 调用 observe 方法，观测整个 data 的变化
- 将 vm._data 属性代理到 vm 上
...
`observe` 的功能就是用来监测数据的变化，它的定义在 `src/core/observer/index.js` 中。`observe` 方法的作用就是给非 VNode 的对象类型数据添加一个 `Observer`
...
defineReactive函数中，我们可以看到实例化了一个 `Dep` 对象。`Dep` 是整个 getter 依赖收集的核心，它的定义在 `src/core/observer/dep.js` 中：
...
export default class Dep {
  ...
  // hc watcher添加新的依赖，在watch的addDep方法中，也会将此依赖添加新的订阅
  depend () {
    if (Dep.target) {
